...
B_red <- B[,-which(names(B) %in% c("m21"))]
# A_full <- data[data$species=="fosA" & is.element(data$locality, matrix$locality[sites_A]),c(37,6:25)]
# A_full <- droplevels(A_full)
# B_full <- data[data$species=="fosB" & is.element(data$locality, matrix$locality[sites_B]),c(37,6:25)]
# B_full <- droplevels(B_full)
...
meanHo_B_red <- apply(Basic_B_red$Ho, 2, mean, na.rm=TRUE)
...
DATA_B_red <- cbind(meanAr_B_red, meanHo_B_red, meanHs_B_red, betw_undir_B, betw_dir_B, clos_undir_B, clos_dir_B, degree_undir_B, updist_B, catch_B, match_B)
colnames(DATA_B_red) <- c("meanAr","meanHo","meanHs","betw_undir","betw_dir","clos_undir","clos_dir","degree","updist","catch","network_match")
DATA_B_red <- data.frame(DATA_B_red)
...
DATA_red <- rbind(DATA_B_red,DATA_A_red)
DATA_red$spec <- as.factor(DATA_red$spec)

rm(list=c("DATA_A_red","DATA_B","DATA_B_red","Ar_A_red","Ar_B", "Basic_A_red","Basic_B","catch_A_red","catch_B"))
...
DATA_red <- DATA_red[ order(DATA_red$updist), ]
DATA_red$log_updist <- log(DATA_red$updist)
DATA_red$log_catch <- log(DATA_red$catch)
DATA_red$std_clos_undir <- stand(DATA_red$clos_undir)
DATA_red$std_clos_dir <- stand(DATA_red$clos_dir)
...
DISTDATA_B_red <- cbind(fst_B_red,dist_B)
colnames(DISTDATA_B_red) <- c("fst","dist")
DISTDATA_B_red <- data.frame(DISTDATA_B_red)

DISTDATA_B$spec <- "B"
DISTDATA_A_red$spec <- "A"
DISTDATA_B_red$spec <- "B"

DISTDATA <- rbind(DISTDATA_B,DISTDATA_A_red)
DISTDATA$spec <- as.factor(DISTDATA$spec)

DISTDATA_red <- rbind(DISTDATA_B_red,DISTDATA_A_red)
DISTDATA_red$spec <- as.factor(DISTDATA_red$spec)
...
DISTDATA_red <- DISTDATA_red[ order(DISTDATA_red$dist), ]

# Prepare DISTDATA with non-zero Fst values
DISTDATA$nonneg_fst <- DISTDATA$fst
DISTDATA$nonneg_fst[which(DISTDATA$fst<0)] <- 0

DISTDATA_red$nonneg_fst <- DISTDATA_red$fst
DISTDATA_red$nonneg_fst[which(DISTDATA_red$fst<0)] <- 0

# Prepare DISTDATA with log-transformed dist values
DISTDATA_red$log_dist <- log(DISTDATA_red$dist)
...
plot(DATA$meanHs[DATA$spec=="B"],DATA_red$meanHs[DATA_red$spec=="B"],asp=1, main="Gfos B: 10 vs. 9 markers", xlab="Hs 10", ylab="Hs 9")
plot(DISTDATA$nonneg_fst[DISTDATA$spec=="B"],DISTDATA_red$nonneg_fst[DISTDATA_red$spec=="B"],asp=1, main="Gfos B: 10 vs. 9 markers", xlab="Fst 10", ylab="Fst 9")

DATA$meanHs[DATA$spec=="B"]==DATA_red$meanHs[DATA_red$spec=="B"]
DISTDATA$nonneg_fst[DISTDATA$spec=="B"]==DISTDATA_red$nonneg_fst[DISTDATA_red$spec=="B"]
